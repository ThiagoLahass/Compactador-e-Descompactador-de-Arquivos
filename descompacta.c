...
int main ( int argc, char * argv[] ) {
    ...
    // printf("%s\n", argv[1]);
    int vetor_frequencia[256], i = 0, tamanho_bitmap = 0;
    ...
    sscanf(argv[1], "%[^.]", nome_arq_saida );

    //verifica a frequencia de cada caractere
    fread(vetor_frequencia, sizeof(int), 256, arquivo_entrada);
    fread(extensao, sizeof(char), 10, arquivo_entrada);
    strcat(nome_arq_saida, extensao);
    ...
    arquivo_saida = fopen( nome_arq_saida, "w");
    if( !arquivo_saida ){
...
void Gera_arquivo_de_saida ( FILE* arquivo_saida, FILE* arquivo_entrada, Arv* arvore_otima, char** tabela_codificacao, int vet_freq[] ){
    int i = 0, tamanho_bitmap = 0;
    char c;
    bitmap* bitmap;
    Arv* arv_aux = arvore_otima;
    ...
    fread(bitmapGetContents(bitmap),sizeof(unsigned char),(tamanho_bitmap+7)/8,arquivo_entrada);

    for (int i=0; i<tamanho_bitmap; i++) {
        u_int32_t teste;
        teste = bitmapGetBit(bitmap,i);
        
        if((0x01 & teste) != 0){
            arv_aux = arv_retornaNoDireita(arv_aux);
        
        }else if((0x01 & teste) == 0){
            arv_aux = arv_retornaNoEsquerdo(arv_aux);
        }
        
        if(arv_ehFolha(arv_aux)){
            char letra = arv_retornaLetra(arv_aux);
            fprintf(arquivo_saida,"%c",letra); 
            arv_aux = arvore_otima;
        }
    }
